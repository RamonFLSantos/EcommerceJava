package br.sistema.persistencia;

import br.sistema.conta.Compra;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule;
import com.fasterxml.jackson.databind.SerializationFeature;


import java.io.File;
import java.io.IOException;
import java.util.ArrayList;

public class PersistenciaCompra {
    private static final String CAMINHO_ARQUIVO = "compras.json";

    public static void salvarCompras(ArrayList<Compra> compras) {
        ObjectMapper mapper = new ObjectMapper();
        mapper.registerModule(new JavaTimeModule());
        mapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
        try {
            mapper.writerWithDefaultPrettyPrinter().writeValue(new File(CAMINHO_ARQUIVO), compras);
        } catch (IOException e) {
            System.out.println("Erro ao salvar compras: " + e.getMessage());
        }
    }

    public static ArrayList<Compra> carregarCompras() {
        ObjectMapper mapper = new ObjectMapper();
        mapper.registerModule(new JavaTimeModule());
        mapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);

        File arquivo = new File(CAMINHO_ARQUIVO);
        try {
            if (!arquivo.exists()) return new ArrayList<>();
            ArrayList<Compra> lista = mapper.readValue(arquivo, new TypeReference<>() {});
            Compra.atualizarContador(lista);
            return lista;
        } catch (IOException e) {
            System.out.println("Erro ao carregar compras: " + e.getMessage());
            return new ArrayList<>();
        }
    }


}
